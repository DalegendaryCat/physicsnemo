...
def test_step(data_dict, model, device, cfg, vol_factors, surf_factors):
    ...
    with torch.no_grad():
        ...
        # Hardcoded to match ETL constants
        ...
        if output_features_vol is not None:
            ...
            # De-normalise: [Ux, Uy, Uz] × V, p × V² × ρ, nut × V × L
        ...
        if output_features_surf is not None:
            ...
            # De-normalise surface pressure: p × V² × ρ
            prediction_surf = (
...
def main(cfg: DictConfig):
    ...
    for count, dirname in enumerate(dirnames_per_gpu):
        ...
        # Get global parameters from config
        global_params_names = list(cfg.variables.global_parameters.keys())
        global_params_reference = {
        ...
        # All cases rotated to North — use scalar magnitude
        inlet_vel = 2.0
        air_density = global_params_reference["air_density"]

        # Build global_params_reference list
        ...
        # Build global_params_values list
        global_params_values_list = []
        for key in global_params_types.keys():
            if key == "inlet_velocity":
                global_params_values_list.append(inlet_vel)  # scalar magnitude
        ...
        global_params_values_list = np.array(global_params_values_list, dtype=np.float32)
        ...
        # Surface: pressure-only force calculation
        if prediction_surf is not None:
            surface_sizes = torch.unsqueeze(surface_sizes, -1)

            # Pressure-only — no shear since surface velocity removed
        ...
        if prediction_vol is not None:
            ...
            l2_labels = ["Ux", "Uy", "Uz", "p", "nut"]
            l2_vals = np.sqrt(l2_error.cpu().numpy()) / np.sqrt(l2_gt.cpu().numpy())
            print("Volume L-2 norm:", dirname, dict(zip(l2_labels, l2_vals)))
            l2_volume_all.append(l2_vals)

        # Save surface prediction — pressure only
        if prediction_surf is not None:
        ...
        # Save volume prediction — U, p, nut to point data
        if prediction_vol is not None:
    ...
    l2_surface_all = np.asarray(l2_surface_all)
    l2_volume_all = np.asarray(l2_volume_all)
