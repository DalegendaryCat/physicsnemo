...
def rotate_back_case(cases_dir, case_name):
    """
    Rotate a case back to its original direction after processing.
    Apply inverse rotation to TIME folder and regenerate VTK files
    """
    case_path = os.path.join(cases_dir, case_name)
    direction = case_name.split("_")[-1].upper()
    if direction not in ['N', 'S']:
    	rotation_angle_back = 360 - ROTATION_MAP.get(direction, 0)
    	
    else:
    	rotation_angle_back = ROTATION_MAP.get(direction, 0)
    	
    if rotation_angle_back == 0:
    	print(f"  [SKIP rotate-back] {case_name} - no rotation needed (N case)")
    	return True
    	
    print(f"  [ROTATE-BACK] {case_name} - Applying Rz={rotation_angle_back} to restore {direction} orientation")
    
    rotate_log_path = os.path.join(case_path, "transformPoints_back.log")
    rotate_cmd = [
    	"transformPoints",
    	"-rotateFields",
    	"-case", case_path,
    	f"Rz={rotation_angle_back}",
    ]
    
    with open(rotate_log_path, "w") as log_file:
    	result = subprocess.run(rotate_cmd, stdout=log_file, stderr=subprocess.STDOUT)
    
    if result.returncode != 0:
    	print(f"  [ERROR] rotate-back failed for {case_name} - check {rotate_log_path}")
    	return False
    
    print(f"  [OK] {case_name} restored to original {direction} orientation")
    return True
...
def main():
    ...
    # ── Rotate back all cases to original orientation ─────────────────────────
    print("\n==========================================")
    print(" Rotating cases back to original orientation")
    print("==========================================\n")
    
    rotate_back_start = time.perf_counter()
    rotate_back_ok = []
    rotate_back_fail = []
    
    for case_name in success:
        ok = rotate_back_case(cases_dir, case_name)
    	if ok:
    	    rotate_back_ok.append(case_name)
    	    
    	else:
    	    rotate_back_fail.append(case_name)
    	
    rotate_back_elapsed = time.perf_counter() - rotate_back_start
    print(f"\n  Rotate-back completed in {rotate_back_elapsed:.1f}s ({rotate_back_elapsed/60:.1f} min")
    print(f"  OK:	{len(rotate_back_ok}")
    print(f"  Failed:  {len(rotate_back_fail)}")
    
    for c in rotate_back_fail:
        print(f"  x {c}")
        
    print("\nAll cases restored to original direction.")
