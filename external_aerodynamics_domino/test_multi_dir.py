...
def test_step(data_dict, model, device, cfg, vol_factors, surf_factors):
    ...
    with torch.no_grad():
        ...
        #     global_params_reference[:, 0, :]**2 +
        #     global_params_reference[:, 1, :]**2
        # )
        # air_density = global_params_reference[:, 2, :]

        # TESTING INFERENCE
        stream_velocity = torch.full_like(global_params_reference[:, 0, :], 2.0)
        air_density = torch.full_like(global_params_reference[:, 0, :], 1.225)

        # ADD THIS
        print(f"stream_velocity: {stream_velocity[0,0].item():.4f}")
        print(f"air_density: {air_density[0,0].item():.4f}")
...
def main(cfg: DictConfig):
    ...
    for count, dirname in enumerate(dirnames_per_gpu):
        ...
        # direction of flow
        direction = dirname.split("_")[-1].upper()
        wind_map = {"N": [0.0, -2.0], "S": [0.0, 2.0], "E": [-2.0, 0.0], "W": [2.0, 0.0]}
        inlet_vel = wind_map[direction]
        ...
        #     print(f"VTP surface U range: min={surface_fields[:, 1:4].min():.4f}, max={surface_fields[:, 1:4].max():.4f}")
        #     print(f"UPred range:         min={prediction_surf[0, :, 1:4].min():.4f}, max={prediction_surf[0, :, 1:4].max():.4f}")
        #     print(f"VTP pressure range:  min={surface_fields[:, 0].min():.4f}, max={surface_fields[:, 0].max():.4f}")
        #     print(f"PPred range:         min={prediction_surf[0, :, 0].min():.4f}, max={prediction_surf[0, :, 0].max():.4f}")

        # if prediction_vol is not None:
        #     print(f"VTU volume U range:  min={volume_fields[:, :3].min():.4f}, max={volume_fields[:, :3].max():.4f}")
        #     print(f"UPred vol range:     min={prediction_vol[0, :, :3].min():.4f}, max={prediction_vol[0, :, :3].max():.4f}")
        # print("=========================\n")

        # # Add this debug print in test.py main loop
        # print(f"Case: {dirname}")
        # print(f"inlet_vel: {inlet_vel}")
        # print(f"global_params_values: {global_params_values_list}")
        ...
        if prediction_vol is not None:
            ...
            # polydata_vol.GetCellData().AddArray(volParam_vtk)
